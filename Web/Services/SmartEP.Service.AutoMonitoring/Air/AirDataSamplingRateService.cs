...
{
    /// <summary>
    /// 名称：AirDataSamplingRateService.cs
    /// 创建人：王秉晟
    /// 创建日期：2015-08-22
    /// 维护人员：
    /// 最新维护人员：
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public class AirDataSamplingRateService : IDataSamplingRate
    {
        /// <summary>
        /// 测点类
        /// </summary>
        ...
        /// <returns></returns>
        public decimal GetLocusOfControlSamplingRate(DateTime dtmStart, DateTime dtmEnd)
        {
            //获取国控点位列表
        ...
        public decimal GetPointSamplingRate(string[] portIds, DateTime dtmStart, DateTime dtmEnd)
        {
            ...
            return GetPointSamplingRate(portIds.ToArray(), factors, dtmStart, dtmEnd);
        }
        ...
        public decimal GetPointSamplingRate(string[] portIds, string[] factors, DateTime dtmStart, DateTime dtmEnd)
        {
            ...
            int shouldCount = ((dtmEnd - dtmStart).Days + 1) * 24;//((dtmEnd - dtmStart).Days * 24) + ((dtmEnd - dtmStart).Hours + 1);//每个因子应测条数
            int allShouldCount = 0;//总应测条数
            decimal allRealCount = 0;//总实测条数
            ...
            if (infectantBy60Service != null)
            {
                ...
                {
                    IList<string> pollutantCodeList = (pointPollutantCodeList.ContainsKey(dtSampRate.Rows[i]["PointId"].ToString()))
                                      ? pointPollutantCodeList[dtSampRate.Rows[i]["PointId"].ToString()] : new List<string>(); //该测点对应的因子Code列
                    if (pollutantCodeList.Contains(dtSampRate.Rows[i]["PollutantCode"].ToString()))
                    {
        ...
        /// <summary>
        /// 获取某一时间段苏州市区所有监测点数据总捕获率
        /// <summary>
        /// <param name="dtmStart">开始时间</param>
        /// <param name="dtmEnd">结束时间</param>
        /// <returns></returns>
        public decimal GetSuZhouSamplingRate(DateTime dtmStart, DateTime dtmEnd)
        {
        ...
        /// <summary>
        /// 获取某一时间段某一城区所有监测点数据总捕获率
        /// <summary>
        /// <param name="urbanAreas">城区数据</param>
        /// <param name="dtmStart">开始时间</param>
        /// <param name="dtmEnd">结束时间</param>
        /// <returns></returns>
        public decimal GetUrbanAreaSamplingRate(IList<string> urbanAreas, DateTime dtmStart, DateTime dtmEnd)
        {
            ...
            return GetPointSamplingRate(portIds.ToArray(), factors.ToArray(), dtmStart, dtmEnd);
        }

        /// <summary>
        /// 获取某一时间段乡镇所有监测点数据总捕获率
        /// <summary>
        /// <param name="villagesTownsList">乡镇数据</param>
        /// <param name="dtmStart">开始时间</param>
        /// <param name="dtmEnd">结束时间</param>
        /// <returns></returns>
        public decimal GetVillagesTownsSamplingRate(IList<string> villagesTownsList, DateTime dtmStart, DateTime dtmEnd)
        {
            IList<string> portIds = new List<string>();
            IList<string> factors = new List<string>();
        ...
        /// <summary>
        /// 获取某一时间段创模点所有监测点数据总捕获率
        /// <summary>
        /// <param name="dtmStart">开始时间</param>
        /// <param name="dtmEnd">结束时间</param>
        /// <returns></returns>
        public decimal GetModelSamplingRate(DateTime dtmStart, DateTime dtmEnd)
        {
            //获取创模点位列表
        ...
        /// </returns>
        public DataView GetLocusOfControlSamplingRateDetail(DateTime dtmStart, DateTime dtmEnd,
            int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Tstamp")
        {
            ...
            try
            {
                ...
                string[] pointIds = GetPointIdsByPointEntitys(entityQueryable.ToArray());//根据测点数组获取测点Id列
                string[] factors = GetPollutantCodesByPointEntitys(entityQueryable.ToArray());//根据测点数组获取因子列
                return GetPointSamplingRateDetail(pointIds, factors, dtmStart, dtmEnd, pageSize, pageNo, out recordTotal, orderBy);
            }
            catch (Exception ex)
            {
                return null;
        ...
        /// 同GetPointSamplingRateDetail
        /// </returns>
        public DataView GetSuZhouSamplingRateDetail(DateTime dtmStart, DateTime dtmEnd, int pageSize, int pageNo,
        ...
        /// 同GetPointSamplingRateDetail
        /// </returns>
        public DataView GetUrbanAreaSamplingRateDetail(IList<string> urbanAreas, DateTime dtmStart, DateTime dtmEnd,
            int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Tstamp")
        {
            recordTotal = 0;
            try
            {
                ...
                return GetPointSamplingRateDetail(portIds.ToArray(), factors.ToArray(), dtmStart, dtmEnd, pageSize, pageNo, out recordTotal, orderBy);
            }
            catch (Exception ex)
            {
                return null;
        ...
        /// 同GetPointSamplingRateDetail
        /// </returns>
        public DataView GetVillagesTownsSamplingRateDetail(IList<string> villagesTownsList,
        ...
        /// 同GetPointSamplingRateDetail
        /// </returns>
        public DataView GetModelSamplingRateDetail(DateTime dtmStart, DateTime dtmEnd,
            int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Tstamp")
        {
            recordTotal = 0;
            try
            ...
            out int recordTotal, string orderBy = "PointId,Tstamp")
        {
            ...
            int shouldDays = (dtmEnd - dtmStart).Days;//天数
            int shouldCount = shouldDays * 24;//每个因子应测条数
            ...
            if (infectantBy60Service != null)
            {
                ...
                for (int i = 0; i < portIds.Length; i++)
                {
                    ...
                    {
                        //根据测点Id获取测点名称
                        MonitoringPointEntity entity = g_MonitoringPointAir.RetrieveEntityByPointId(pointId);
                        ...
                        if (rowShouldCount > 0)
                        {
                            ...
                            {
                                if (pollutantCodeList.Contains(drPointData["PollutantCode"].ToString()))//如果该测点有该因子，累加实测条数
                                {
        ...
        {
            DateTime dtmEnd = DateTime.Now;
            DateTime dtmStart = dtmEnd.AddHours(-hours);
            decimal shouldDays = (decimal)(dtmEnd - dtmStart).TotalDays;//天数
            int shouldCount = hours;//每个因子应测条数
            int totalRealCount = 0;//合计实测条数
            int totalShouldCount = 0;//合计应测条数
            DataTable dtNew = new DataTable();//新建表
            string[] factors;
            ...
            out int recordTotal, string orderBy = "PointId,Tstamp")
        {
            ...
            dtmStart = new DateTime(dtmStart.Year, dtmStart.Month, dtmStart.Day, 0, 0, 0);
            dtmEnd = new DateTime(dtmEnd.Year, dtmEnd.Month, dtmEnd.Day, 23, 59, 59);
            int shouldCount = (int)Math.Floor(shouldDays * 24);
            int totalRealCount = 0;//合计实测条数
            int totalShouldCount = 0;//合计应测条数
            ...
            , DateTime dtmStart, DateTime dtmEnd, int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId")
        {
            ...
            if (infectantBy60Service != null)
            {
                ...
                for (int i = 0; i < dtSampRate.Rows.Count; i++)
                {
                    ...
                    int totalRealCount = 0;//合计实测条数
                    int totalShouldCount = 0;//合计应测条数
                    decimal samplingRate = Math.Round((decimal)realCount * 100 / shouldCount, 2);//捕获率
        ...
        public DataView GetSamplingRateStatisticalData(string[] portIds, string[] factors, DateTime dtmStart, DateTime dtmEnd)
        {
            ...
            dtmEnd = new DateTime(dtmEnd.Year, dtmEnd.Month, dtmEnd.Day, 23, 59, 59);
            DataTable dtNew = new DataTable();//新建表
            int shouldCount = ((dtmEnd - dtmStart).Days + 1) * 24;//((dtmEnd - dtmStart).Days * 24) + ((dtmEnd - dtmStart).Hours + 1);//每个测点的因子应测条数
