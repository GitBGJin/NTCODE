...
{
    /// <summary>
    /// 名称：WaterDataEffectRateNewService.cs
    /// 创建人：尤红兵
    /// 创建日期：2015-12-17
    /// 维护人员：
    /// 最新维护人员：
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public class WaterDataSamplingRateNewService : BaseGenericRepository<MonitoringBusinessModel, ReportSamplingRateByDayEntity>
    {
        ...
        {
            DataTable dtDataSamplingRate = ConvertToDataTable(Retrieve(it => it.ReportDateTime >= dtmStart && it.ReportDateTime <= dtmEnd && portIds.Contains(it.PointId.ToString()) && factors.Contains(it.PollutantCode) && it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr"));
            ...
            foreach (string point in portIds)
            {
                ...
                foreach (string factor in factors)
                {
                    ...
                    if (drDataSamplingRate != null && drDataSamplingRate.Count() > 0 )
                    {
                        ...
                        {
                            //捕获率
                            percentageValue = string.Format("{0:N2}", (actualValue / sampleValue) * 100 > 100 ? 100 : (actualValue / sampleValue) * 100);
        ...
        public decimal DataSamplingRate(string[] pointIds, DateTime starttime, DateTime endtime)
        {
            DataTable dtDataSamplingRate = ConvertToDataTable(Retrieve(it => it.ReportDateTime >= starttime && it.ReportDateTime <= endtime && pointIds.Contains(it.PointId.ToString()) && it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr"));
            DataView dv = dtDataSamplingRate.DefaultView;
            decimal samplingrate = 0;
            decimal actualValue = 0;
            decimal sampleValue = 0;
            foreach (string portid in pointIds)
            {
                ...
                dv.RowFilter = "pointId =" + portid + " and ReportDateTime>='" + starttime + "' and ReportDateTime<='" + endtime + "' and PollutantCode in (" + factors + ")";
                for (int i = 0; i < dv.Count; i++)
                {
                    if (dv[i]["ActualCollectionNumber"] != DBNull.Value)
                    {
                        actualValue += Convert.ToDecimal(dv[i]["ActualCollectionNumber"]);//实际测试值
                    }
                    if (dv[i]["SamplingNumber"] != DBNull.Value)
                    {
                        sampleValue += Convert.ToDecimal(dv[i]["SamplingNumber"]);//应该测试值
                    }
                }
            }
            if (sampleValue != 0)
            {
                samplingrate = DecimalExtension.GetPollutantValue(actualValue / sampleValue * 100, 2);
            }
            return samplingrate;
        }

        public DataTable GetdtDataSamplingRate(string[] portIds, string[] factors, DateTime dtmStart, DateTime dtmEnd, string orderBy = "PointId,Tstamp")
        {
            DataTable dtDataSamplingRate = ConvertToDataTable(Retrieve(it => it.ReportDateTime >= dtmStart && it.ReportDateTime <= dtmEnd && portIds.Contains(it.PointId.ToString()) && factors.Contains(it.PollutantCode) && it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr"));
            return dtDataSamplingRate;
        }
        /// <summary>
        /// 获取捕获率数据（首页原件）
        /// </summary>
        ...
        /// <param name="dtmEnd">结束时间</param>
        /// <returns></returns>
        public DataTable DataSamplingRateRetrieve(string[] portIds, string[] factors, DateTime dtmStart, DateTime dtmEnd,
            int pageSize, int pageNo, string orderBy = "PointId,Tstamp")
        {
            DataTable dtDataSamplingRate = ConvertToDataTable(Retrieve(it => it.ReportDateTime >= dtmStart && it.ReportDateTime <= dtmEnd && portIds.Contains(it.PointId.ToString()) && factors.Contains(it.PollutantCode) && it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr"));
            DataTable dtHomePageTable = CreateHomePageTable();//生成一个临时表 保存计算的捕获率数据
            MonitoringPointWaterService monitoringPointWater = new MonitoringPointWaterService();
            //按站点和因子计算数据
            foreach (string point in portIds)
            {
                DataRow drHomePageTable = dtHomePageTable.NewRow();
                MonitoringPointEntity monitoringPoint = monitoringPointWater.RetrieveEntityByPointId(Convert.ToInt32(point));

                if (monitoringPoint != null)
                {
                    drHomePageTable["PointName"] = monitoringPoint.MonitoringPointName;//测点名称
                }
                else
                {
                    drHomePageTable["PointName"] = "";
                }

                decimal totalActualValue = 0;//实际测试值
                decimal totalSampleValue = 0;//应该测试值
                foreach (string factor in factors)
                {
                    DataRow[] drDataSamplingRate = dtDataSamplingRate.Select(string.Format("PointId='{0}' and PollutantCode='{1}'", point, factor));
                ...
                drHomePageTable["TotalShouldCount"] = totalSampleValue;
                drHomePageTable["TotalRealCount"] = totalActualValue;
                if (totalSampleValue != 0)
                {
                    drHomePageTable["CaptrureRate"] = string.Format("{0:N2}", (totalActualValue / totalSampleValue) * 100);
                }
                else
                {
                    drHomePageTable["CaptrureRate"] = 0;
                }
        ...
        private DataTable CreateHomePageTable()
        {
            ...
            dtNew.Columns.Add("TotalShouldCount", typeof(decimal));//应测数据
            dtNew.Columns.Add("TotalRealCount", typeof(decimal));//实测数据
            dtNew.Columns.Add("CaptrureRate", typeof(decimal));//捕获率
        ...
        public DataView GetSamplingDetailData(string[] portIds, string[] factors, DateTime dtmStart, DateTime dtmEnd)
        {
            ...
            DataTable dtSamplingDetailData = ConvertToDataTable(Retrieve(it => it.ReportDateTime >= dtmStart && it.ReportDateTime <= dtmEnd && portIds.Contains(it.PointId.ToString()) && factors.Contains(it.PollutantCode) && it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr"));
