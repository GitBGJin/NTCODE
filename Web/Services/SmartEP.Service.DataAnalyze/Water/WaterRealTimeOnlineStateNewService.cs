...
{
    public class WaterRealTimeOnlineStateNewService : BaseGenericRepository<MonitoringBusinessModel, V_DataSamplingConditionEntity>
    {
        ...
        /// <returns></returns>
        public DataView GetRealTimeOnlineStateDataPager(string[] portIds, string[] factors, string netWorkType, Dictionary<string, int> dicStatusCode, out DataView dvOnlineRate, int pageSize, int pageNo, out int recordTotal,
            string orderBy = "PointId,Tstamp")
        {
            ...
            {
                //dtDataOnlineState = ConvertToDataTable(Retrieve(it => it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr" && portIds.Contains(it.PointId.ToString())));
                //这里包括在线和离线 如果获取全部状态使用上面的方法
                dtDataOnlineState = ConvertToDataTable(Retrieve(it => it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr" && portIds.Contains(it.PointId.ToString()) && (it.StatusCode == "0" || it.StatusCode == "1")));
            }
            else
            {
                dtDataOnlineState = ConvertToDataTable(Retrieve(it => it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr" && portIds.Contains(it.PointId.ToString()) && it.StatusCode == netWorkType));
            }
            ...
            if (dtDataOnlineState != null && dtDataOnlineState.Rows.Count > 0)
            {
                ...
                {
                    drStatisticsTable["OnlineRate"] = (Convert.ToDecimal(drStatisticsTable["OnlineCount"]) / AllCount) * 100;
                }
                else
                {
                ...
                #region list页面
                //获取因子数据
                DataView dvWaterRecentTimeData = g_InfectantBy60Repository.GetWaterRecentTimeData(portIds, factors);
                //去掉因子的引号
        ...
        public DataTable GetRealTimeOnlineStateDataPager(string[] portIds, Dictionary<string, int> dicStatusCode)
        {
            ...
            DataTable dtDataOnlineState = ConvertToDataTable(Retrieve(it => it.ApplicationUid == "watrwatr-watr-watr-watr-watrwatrwatr" && portIds.Contains(it.PointId.ToString())));
            if (dtDataOnlineState != null && dtDataOnlineState.Rows.Count > 0)
            {
                int AllCount = 0;
