...
{
    /// <summary>
    /// 名称：WaterRealTimeOnlineStateService.cs
    /// 创建人：王秉晟
    /// 创建日期：2015-08-23
    /// 维护人员：
    /// 最新维护人员：
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public class WaterRealTimeOnlineStateService : IRealTimeOnline
    {
        /// <summary>
        /// 测点类
        /// </summary>
             ...
             string orderBy = "PointId,Tstamp")
        {
            ...
            if (g_IInfectantRepository != null)
            {
                ...
                foreach (DataRow drLastPort in dtLastPort.Rows)
                {
                    ...
                    TimeSpan timeSpan = new TimeSpan(7, dtmNow.Minute, dtmNow.Second);//允许的时间差
                    ...
                    string statusCode = pointStatusList.ContainsKey(drLastPort["PointId"].ToString())
                        ? pointStatusList[drLastPort["PointId"].ToString()] : string.Empty;

                    ////该测点对应的因子Code列
                    //IList<string> pollutantCodeList = (pointPollutantCodeList.ContainsKey(drLastPort["PointId"].ToString()))
                    //               ? pointPollutantCodeList[drLastPort["PointId"].ToString()] : new List<string>();
        ...
        public DataView GetRealTimeOnlineStateDataPager(string netWorkType, string[] portIds, string[] factors,
               int pageSize, int pageNo, out int recordTotal, out DataView dvOnlineRate, string orderBy = "PointId,Tstamp")
        {
        ...
        public DataView GetRealTimeOnlineInfoData()
        {
            ...
            int onlineCount = dataSamplingConditionService.RetrieveWaterOnline().Count();
            int offlineCount = dataSamplingConditionService.RetrieveWaterOffline().Count();
            int alwaysOnlineCount = dataSamplingConditionService.RetrieveWaterAlwaysOnline().Count();
            int totalCount = dataSamplingConditionService.RetrieveWaterAll().Count();
        ...
        public DataView GetRunningStateInfoData(string[] portIds)
        {
            ...
            int onlineCount = (dataSamplingConditionService.RetrieveWaterOnline() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int offlineCount = (dataSamplingConditionService.RetrieveWaterOffline() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int alarmCount = (dataSamplingConditionService.RetrieveWaterAlarm() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int failureCount = (dataSamplingConditionService.RetrieveWaterFailure() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int stopCount = (dataSamplingConditionService.RetrieveWaterStop() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int alwaysOnlineCount = (dataSamplingConditionService.RetrieveWaterAlwaysOnline() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            int totalCount = (dataSamplingConditionService.RetrieveWaterAll() as IQueryable<DataSamplingConditionEntity>)
                              .Count(t => acquisitionUidList.Contains(t.AcquisitionUid));
            drNew["OnlineCount"] = onlineCount;
            drNew["OfflineCount"] = offlineCount;
        ...
        private IList<string> GetAcquisitionUidsByPointId(string[] pointIds)
        {
            ...
            IQueryable<MonitoringPointEntity> pointQueryable = g_MonitoringPointWater.RetrieveListByPointIds(pointIds);
        ...
        private Dictionary<string, string> GetStatusCodeByPointId(string[] pointIds)
        {
            ...
            IQueryable<DataSamplingConditionEntity> dataSamplingConditionEntityQueryable = dataSamplingConditionService.RetrieveWaterAll()
                                            as IQueryable<DataSamplingConditionEntity>;//取得水站所有测点数据采集情况
        ...
        private Dictionary<string, string> GetPointsByPointTypeUids(string[] pointTypeUids)
        {
            ...
            foreach (string pointTypeUid in pointTypeUids)
            {
                ...
                IQueryable<MonitoringPointEntity> entityQueryable = g_MonitoringPointWater.RetrieveWaterMPList(null, null, pointTypeUid);
        ...
        private DataTable CreateNewDataTable(DataTable dtOld)
        {
            ...
            if (dtOld != null)
            {
                ...
                dtNew.Columns.Add("NetWorking", typeof(string));//联网
