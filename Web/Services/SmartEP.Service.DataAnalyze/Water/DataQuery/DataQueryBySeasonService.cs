...
namespace SmartEP.Service.DataAnalyze.Water.DataQuery
{
    ...
    public class DataQueryBySeasonService
    {
        ...
        public DataView GetSeasonStatisticalData(string[] portIds, string[] factors, int yearFrom, int seasonOfYearFrom
           , int yearTo, int seasonOfYearTo)
        {
            if (factors.IsNotNullOrDBNull())
            {
                g_MonitoringPointWater = Singleton<MonitoringPointWaterService>.GetInstance();
                DataTable dt = dt = SeasonData.GetStatisticalData(portIds, factors, yearFrom, seasonOfYearFrom, yearTo, seasonOfYearTo).Table;
                dt.Columns.Add("portName", typeof(string));
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    int pointId = Convert.ToInt32(dt.Rows[i]["PointId"]);
        ...
        /// <param name="orderBy">排序方式（字段：PointId,Year,SeasonOfYear）</param>
        /// <returns></returns>
        public DataView GetSeasonExportData(string[] portIds, string[] factors, int yearFrom, int seasonOfYearFrom
            , int yearTo, int seasonOfYearTo, string orderBy = "PointId,Year,SeasonOfYear")
        {
            if (SeasonData != null)
                return SeasonData.GetExportData(portIds, factors, yearFrom, seasonOfYearFrom, yearTo, seasonOfYearTo, orderBy);
