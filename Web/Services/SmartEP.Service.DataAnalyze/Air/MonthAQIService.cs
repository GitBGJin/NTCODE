    ...
    public class MonthAQIService : IDayAQI
    {
        ...
        public DataView GetDataMonthPager(DateTime dtStart, DateTime dtEnd)
        {
            ...
            if (dv.Count > 0)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    decimal value = DecimalExtension.GetRoundValue(Convert.ToDecimal(dv[0][j]), 4);
            ...
            if (dvT.Count > 0)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    decimal value = DecimalExtension.GetRoundValue(Convert.ToDecimal(dvT[0][j]), 4);
                    decimal temp = DecimalExtension.GetRoundValue(s_AQIService.GetAQI(factorCode, value, num), 0);
                    if (max < temp)
                    {
                        max = temp;
            ...
            if (dvN.Count > 0)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    decimal value = DecimalExtension.GetRoundValue(Convert.ToDecimal(dvN[0][j]), 4);
                    decimal temp = DecimalExtension.GetRoundValue(s_AQIService.GetAQI(factorCode, value, num), 0);
                    if (max < temp)
                    {
                        max = temp;
            ...
            if (dvNew.Count > 0)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    decimal value = DecimalExtension.GetRoundValue(Convert.ToDecimal(dvNew[0][j]), 4);
                    decimal temp = DecimalExtension.GetRoundValue(s_AQIService.GetAQI(factorCode, value, num), 0);
                    if (max < temp)
                    {
                        max = temp;
        ...
        public DataSet GetRegionsMonthAllData(DateTime dateStart, DateTime dateEnd, string strYearJiShu)
        {
            ...
            if (regionDayAQI != null && regionDayAQIRepository != null && dayReportRepository != null)
            {
                ...
                #region AQI范围
                decimal nowMinAQIQuXian = decimal.TryParse(dtNowMinValueQuXian.Compute("MIN(AQIValue)", "").ToString(), out nowMinAQIQuXian) ? nowMinAQIQuXian : 0;//最小值
                decimal nowMaxAQIQuXian = decimal.TryParse(dtNowMaxValueQuXian.Compute("MAX(AQIValue)", "").ToString(), out nowMaxAQIQuXian) ? nowMaxAQIQuXian : 0;//最大值
                ...
                if (drsNowMonth.Length > 0)
                {
                    ...
                    decimal optimal = decimal.TryParse(drNowMonth["Optimal"].ToString(), out validCount) ? validCount : 0;//优的天数
                    decimal benign = decimal.TryParse(drNowMonth["benign"].ToString(), out validCount) ? validCount : 0;//良的天数
                    decimal lightPollution = decimal.TryParse(drNowMonth["LightPollution"].ToString(), out validCount) ? validCount : 0;//轻度污染的天数
                    decimal moderatePollution = decimal.TryParse(drNowMonth["ModeratePollution"].ToString(), out validCount) ? validCount : 0;//中度污染的天数
                    decimal highPollution = decimal.TryParse(drNowMonth["HighPollution"].ToString(), out validCount) ? validCount : 0;//重度污染的天数
                    decimal seriousPollution = decimal.TryParse(drNowMonth["SeriousPollution"].ToString(), out validCount) ? validCount : 0;//严重污染的天数
                    ...
                    primaryDaysDictionary.Add("PM2.5", Convert.ToInt32(PM25_OverCount));
                    primaryDaysDictionary.Add("PM10", Convert.ToInt32(PM10_OverCount));
                    primaryDaysDictionary.Add("NO2", Convert.ToInt32(NO2_OverCount));
                    primaryDaysDictionary.Add("SO2", Convert.ToInt32(SO2_OverCount));
                    primaryDaysDictionary.Add("CO", Convert.ToInt32(CO_OverCount));
                    primaryDaysDictionary.Add("O3日最大8小时", Convert.ToInt32(O3_OverCount));
                    ...
                    drText["M102_ChaoFactorTwo"] = overDaysTwo.Aggregate((a, b) => a + "、" + b);
                    #endregion
                ...
                if (dtNowMonth.Rows.Count > 0)
                {
                    ...
                    //所有区县值算全市平均值
                    decimal PM25_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a34004)", "").ToString(), out PM25_CQS) ? PM25_CQS : 0) * 1000, 1);
                    decimal PM10_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a34002)", "").ToString(), out PM10_CQS) ? PM10_CQS : 0) * 1000, 0);
                    decimal NO2_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a21004)", "").ToString(), out NO2_CQS) ? NO2_CQS : 0) * 1000, 0);
                    decimal SO2_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a21026)", "").ToString(), out SO2_CQS) ? SO2_CQS : 0) * 1000, 0);
                    decimal CO_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a21005)", "").ToString(), out CO_CQS) ? CO_CQS : 0), 2);
                    decimal O3_CQS = DecimalExtension.GetRoundValue((decimal.TryParse(dtNowMonthQuXian.Compute("AVG(a05024)", "").ToString(), out O3_CQS) ? O3_CQS : 0) * 1000, 0);
                    decimal standardRate = 0;//达标天数比例
                    decimal standardCountQS = 0;//全市达标天数
                    decimal validCountQS = 0;//全市有效天数
                    decimal standardCountLYQS = 0;//全市上一年月达标天数
                    ...
                    ////表1  XXXX年XX月苏州市环境空气质量
                    //drBiao1["MonitoringRegionUid", typeof(string));//区域GUID 
                    //RegionEn", typeof(string));//英文名称
                    //RegionName", typeof(string));//中文名称
                    //DaBiaoDays", typeof(string));//达标天数
                    //DaBiaoRate", typeof(string));//达标天数比例
                    //DaBiaoLastRate", typeof(string));//上年同月达标天数比例
                    //PM25ChaoDays", typeof(string));//PM2.5超标天数
                    //PM10ChaoDays", typeof(string));//PM10超标天数
                    //NO2ChaoDays", typeof(string));//NO2超标天数
                    //SO2ChaoDays", typeof(string));//SO2超标天数
                    //COChaoDays", typeof(string));//CO超标天数
                    //O38ChaoDays", typeof(string));//O3-8小时超标天数


                    ////表2 XXXX年XX月苏州市各项污染物月均值
                    //drBiao2["MonitoringRegionUid"] = monitoringRegionUid;
                    //drBiao2["RegionEn"] = GetRegionENByGuid(monitoringRegionUid);
                    //drBiao2["RegionName"] = drNowMonthQuXian["regionName"].ToString();
                    //drBiao2["PM25"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a34004"]), 1);
                    //drBiao2["PM10"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a34002"]), 0);
                    //drBiao2["NO2"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a21004"]), 0);
                    //drBiao2["SO2"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a21026"]), 0);
                    //drBiao2["CO"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a21005"]), 2);
                    //drBiao2["O38"] = DecimalExtension.GetRoundValue(Convert.ToDecimal(drNowMonthQuXian["a05024"]), 0);
                    //dtBiao2.Rows.Add(drBiao2);
                    //}

                    ////表2 苏州全市的浓度
                    //DataRow drBiao2QS = dtBiao2.NewRow();
                    //drBiao2QS["MonitoringRegionUid"] = "";
                    //drBiao2QS["RegionEn"] = GetRegionENByGuid("");
                    //drBiao2QS["RegionName"] = "全市";
                    //drBiao2QS["PM25"] = PM25_CQS.ToString();
                    //drBiao2QS["PM10"] = PM10_CQS.ToString();
                    //drBiao2QS["NO2"] = NO2_CQS.ToString();
                    //drBiao2QS["SO2"] = SO2_CQS.ToString();
                    //drBiao2QS["CO"] = CO_CQS.ToString();
                    //drBiao2QS["O38"] = O3_CQS.ToString();
                    //dtBiao2.Rows.Add(drBiao2QS);
                    #endregion

                    #region 全市达标率以及首要污染物最多和第二多的因子
                    if (dtNowQuanShi.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtNowQuanShi.Rows.Count; i++)
                        {
                            ...
                            if (AQIcount > 0)
                            {
                                ...
                                //首要污染物天数
                                if (!string.IsNullOrWhiteSpace(primaryPollutantName) && AQIcount > 100)
                                {
                                    if (primaryDaysQSDictionary.ContainsKey(primaryPollutantName))
                                    {
                                        int primaryDays = primaryDaysQSDictionary[primaryPollutantName];
                                        primaryDays += 1;
                                        primaryDaysQSDictionary[primaryPollutantName] = primaryDays;
                                    }
                                    else
                                    {
                                        primaryDaysQSDictionary.Add(primaryPollutantName, 1);
                                    }
                                }
                            }
                            #endregion
                        ...
                        if (primaryDaysTwo.Count > 0)
                            drText["M101_ChaoFactorTowQuanShi"] = primaryDaysTwo.Aggregate((a, b) => a + "、" + b);
                        #endregion
                    }
                    #endregion
                    ...
                    foreach (DataRow drNowMonthQuXian in dtNowMonthQuXian.Rows)
                    {
                        ...
                        #region 表2 XXXX年XX月苏州市各项污染物月均值
                        decimal PM25Biao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a34004"].ToString(), out PM25Biao2) ? PM25Biao2 : 0) * 1000, 1);
                        decimal PM10Biao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a34002"].ToString(), out PM10Biao2) ? PM10Biao2 : 0) * 1000, 0);
                        decimal NO2Biao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a21004"].ToString(), out NO2Biao2) ? NO2Biao2 : 0) * 1000, 0);
                        decimal SO2Biao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a21026"].ToString(), out SO2Biao2) ? SO2Biao2 : 0) * 1000, 0);
                        decimal COBiao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a21005"].ToString(), out COBiao2) ? COBiao2 : 0), 2);
                        decimal O38Biao2 = DecimalExtension.GetRoundValue((decimal.TryParse(drNowMonthQuXian["a05024"].ToString(), out O38Biao2) ? O38Biao2 : 0) * 1000, 0);
                        drBiao2["MonitoringRegionUid"] = monitoringRegionUid;//区域GUID 
                        drBiao2["RegionEn"] = GetRegionENByGuid(monitoringRegionUid);//英文名称
                        drBiao2["RegionName"] = regionName;//中文名称
                        drBiao2["PM25"] = PM25Biao2.ToString("0.0");//PM2.5浓度
                        drBiao2["PM10"] = PM10Biao2.ToString("0");//PM10浓度
                        drBiao2["NO2"] = NO2Biao2.ToString("0");//NO2浓度
                        drBiao2["SO2"] = SO2Biao2.ToString("0");//SO2浓度
                        drBiao2["CO"] = COBiao2.ToString("0.00");//CO浓度
                        drBiao2["O38"] = O38Biao2.ToString("0");//O38浓度
                        dtBiao2.Rows.Add(drBiao2);
                        #endregion
                    }
                    ...
                    if (isLastM)
                    {
                        ...
                        //所有区县值算全市平均值
                        decimal PM25_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a34004)", "").ToString(), out PM25_CLM) ? PM25_CLM : 0) * 1000, 1);
                        decimal PM10_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a34002)", "").ToString(), out PM10_CLM) ? PM10_CLM : 0) * 1000, 0);
                        decimal NO2_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a21004)", "").ToString(), out NO2_CLM) ? NO2_CLM : 0) * 1000, 0);
                        decimal SO2_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a21026)", "").ToString(), out SO2_CLM) ? SO2_CLM : 0) * 1000, 0);
                        decimal CO_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a21005)", "").ToString(), out CO_CLM) ? CO_CLM : 0), 2);
                        decimal O3_CLM = DecimalExtension.GetRoundValue((decimal.TryParse(dtLastMonthQuXian.Compute("AVG(a05024)", "").ToString(), out O3_CLM) ? O3_CLM : 0) * 1000, 0);
                        IList<string> upFactorList = new List<string>();
                        IList<string> equalFactorList = new List<string>();
                        IList<string> downFactorList = new List<string>();
        ...
        private DataSet CreateMonthReportDataTable()
        {
            ...
            DataTable dtBiao2 = new DataTable("Biao2");//表2  XXXX年XX月苏州市各项污染物月均值
            DataTable dtBiao3 = new DataTable("Biao3");//表3  XXXX年XX月苏州市降水监测结果统计
            ...
            ds.Tables.AddRange(new DataTable[] { dtText, dtBiaoAirInfo, dtBiao1, dtBiao2, dtBiao3, dtTu1, dtTu2 });
            ...
            dtBiao2.Columns.Add("MonitoringRegionUid", typeof(string));//区域GUID 
            dtBiao2.Columns.Add("RegionEn", typeof(string));//英文名称
            dtBiao2.Columns.Add("RegionName", typeof(string));//中文名称
            dtBiao2.Columns.Add("PM25", typeof(string));//PM2.5浓度
            dtBiao2.Columns.Add("PM10", typeof(string));//PM10浓度
            dtBiao2.Columns.Add("NO2", typeof(string));//NO2浓度
            dtBiao2.Columns.Add("SO2", typeof(string));//SO2浓度
            dtBiao2.Columns.Add("CO", typeof(string));//CO浓度
            dtBiao2.Columns.Add("O38", typeof(string));//O38浓度
            ...
            dtBiao3.Columns.Add("YangPin", typeof(string));//样品数(个)
            dtBiao3.Columns.Add("Min", typeof(string));//pH最小值
            dtBiao3.Columns.Add("Max", typeof(string));//pH最大值
            dtBiao3.Columns.Add("Avg", typeof(string));//降水pH均值
            dtBiao3.Columns.Add("PinLv", typeof(string));//酸雨频率(%)
            ...
            ref IList<string> downFactorList)
        {
            ...
            if (upFactorCount == equalFactorCount && equalFactorCount == downFactorCount)
            {
