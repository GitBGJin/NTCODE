...
namespace SmartEP.Data.SqlServer.MonitoringBusiness.Air
{
    ...
    public class YearReportDAL
    {
        ...
        /// 数据库表名
        /// </summary>
        private string tableName = "AirReport.TB_YearReport";
        #endregion
        ...
        /// 构造函数
        /// </summary>
        public YearReportDAL()
        {
            tableName = SmartEP.Data.Enums.EnumMapping.GetAutoMonitorTableName(ApplicationType.Air, PollutantDataType.Year);
            connection = SmartEP.Data.Enums.EnumMapping.GetConnectionName(ApplicationType.Air, PollutantDataType.Year);
        }
        #endregion
        ...
        {
            orderBy = !string.IsNullOrEmpty(orderBy) ? orderBy : "PointId,Year";
            recordTotal = 0;
            ...
            orderBy = string.IsNullOrEmpty(orderBy) ? "PointId,Year" : orderBy;
            string fieldName = "PointId,Year" + factorSql + factorFlagSql;
            string groupBy = "PointId,Year";
            string where = string.Format(" Year>='{0}' AND Year<='{1}' ", yearFrom, yearTo) + portIdsStr;
            ...
            , int yearFrom, int yearTo, int pageSize, int pageNo, out int recordTotal, string orderBy )
        {
          ...
          orderBy = !string.IsNullOrEmpty(orderBy) ? orderBy : "sortNumber desc,Year";
          recordTotal = 0;
          ...
          string fieldName = "PointId,Year" + factorSql;
          string newFiledName = " Max(t2.Region) as PointId,Year, sortNumber" + factorNewSql;
          string groupBy = "PointId,Year";
          string where = string.Format(" Year>='{0}' AND Year<='{1}' AND PollutantCode in ({2}) ", yearFrom, yearTo,factorWhere) + portIdsStr;
          tabledt = "(select PointId,Year,d1.PollutantCode,cast([dbo].[F_Round](d1.PollutantValue,case when d2.DecimalDigit is null then 3 when d2.DecimalDigit='' then 3 else d2.DecimalDigit end) as decimal(18,4)) as PollutantValue from [AMS_MonitorBusiness].[AirReport].[TB_YearReport] as d1 left join [AMS_BaseData].[dbo].[V_Factor] as d2 on d1.PollutantCode=d2.PollutantCode) as dt";
          ...
          group by t2.RegionUid,t1.Year,svcRG.sortNumber", newFiledName, table1, "[AMS_BaseData].[dbo].[V_Point_UserConfigNew]");
            ...
            , int yearFrom, int yearTo, int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Year")
        {
            ...
            //取得查询行转列字段拼接
            string factorAsql = "Select Year";
            string factorSql = "select PointId,Year";
            string factorFlagSql = string.Empty;
            foreach (string factor in factors)
            {
            ...
            string groupBy = "PointId,Year";
            string where = string.Format(" Year>='{0}' AND Year<='{1}' ", yearFrom, yearTo) + portIdsStr;
            string sql = string.Format(@"{0} from ({1} from {2} where {3} group by {4})data  group by Year order by Year asc",factorAsql,factorSql,tableName,where,groupBy);
            //return g_GridViewPager.GetPivotDataPager(tableName, fieldName, groupBy, orderBy, where, pageSize, pageNo, connection, out recordTotal);
            ...
            , int yearFrom, int yearTo, int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Year")
        {
            ...
            try
            {
                ...
                orderBy = !string.IsNullOrEmpty(orderBy) ? orderBy : "Year";
                string fieldName = "PointId,Year" + factorSql;
                string where = string.Format(" Year>='{0}' AND Year<='{1}' AND PollutantCode in ({2})", yearFrom, yearTo, factorWhere) + portIdsStr;
                string sql = string.Format(@"select {5}, Year
											{3}
											from(
											select {1}
											 from {0}
            ...
            , int yearFromB, int yearToB, int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Year")
        {
            ...
            orderBy = string.IsNullOrEmpty(orderBy) ? "PointId,Year" : orderBy;
            string fieldName = "PointId,'审核' as Type,Year" + factorSql + factorFlagSql;
            string groupBy = "PointId,Year";
            string where = "";
            if (yearFromB > 0 && yearToB > 0)
                where = string.Format(" ((Year>='{0}' AND Year<='{1}') or (Year>='{2}' AND Year<='{3}')) ", yearFrom, yearTo, yearFromB, yearToB) + portIdsStr;
            else
                where = string.Format(" Year>='{0}' AND Year<='{1}' ", yearFrom, yearTo) + portIdsStr;
            return g_GridViewPager.GetPivotDataPager(tableName, fieldName, groupBy, orderBy, where, pageSize, pageNo, connection, out recordTotal);
        }
            ...
            ,  int pageSize, int pageNo,  string orderBy = "PointId,Year")
        {
            ...
            orderBy = string.IsNullOrEmpty(orderBy) ? "PointId,Year" : orderBy;
            string fieldName = "row_number() over(order by PointId,Year) as Ordernum ,PointId,'审核' as Type,Year" + factorSql + factorFlagSql;
            string groupBy = "PointId,Year";
            string where = "";
            //if (yearFromB > 0 && yearToB > 0)
            //    where = string.Format(" ((Year>='{0}' AND Year<='{1}') or (Year>='{2}' AND Year<='{3}')) ", yearFrom, yearTo, yearFromB, yearToB) + portIdsStr;
            ...
            , int yearFrom, int yearTo, string orderBy = "PointId,Year")
        {
            ...
            {
                string decimalNum = string.IsNullOrEmpty(factor.PollutantDecimalNum) ? "3" : factor.PollutantDecimalNum;
            ...
            //}
            orderBy = string.IsNullOrEmpty(orderBy) ? "PointId,Year" : orderBy;
            string fieldName = "rowNum as '序号',PointId,Year as '年' " + factorSql + factorFlagSql;
            string groupBy = "PointId,Year";
            string where = string.Format(" Year>='{0}' AND Year<='{1}' ", yearFrom, yearTo) + portIdsStr;
            return g_GridViewPager.GetPivotDataPager(tableName, fieldName, groupBy, orderBy, where, pageSize, pageNo, connection, out recordTotal);
        }
        ...
        public int GetAllDataCount(string[] portIds, int yearFrom, int yearTo)
        {
            ...
            string whereString = string.Format(" Year>='{0}' AND Year<='{1}' {2} ", yearFrom, yearTo, portIdsStr);

            return g_GridViewPager.GetAllDataCount(tableName, "PointId,Year", whereString, connection);
        }
        #endregion
    }
}
