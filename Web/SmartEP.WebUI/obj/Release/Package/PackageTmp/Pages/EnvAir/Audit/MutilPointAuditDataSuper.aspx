<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MutilPointAuditDataSuper.aspx.cs" Inherits="SmartEP.WebUI.Pages.EnvAir.Audit.MutilPointAuditDataSuper" %>
...
<body scroll="no">
    <form id="form1" runat="server">
            ...
            <AjaxSettings>
                ...
                <telerik:AjaxSetting AjaxControlID="refreshData">
                    <UpdatedControls>
                        ...
                        <telerik:AjaxUpdatedControl ControlID="InsIDHidden"></telerik:AjaxUpdatedControl>
                        <%--<telerik:AjaxUpdatedControl ControlID="pagediv" LoadingPanelID="RadAjaxLoadingPanel1"></telerik:AjaxUpdatedControl>--%>
                    </UpdatedControls>
                </telerik:AjaxSetting>
                ...
                <telerik:AjaxSetting AjaxControlID="RadMenuAuditLog">
                    <UpdatedControls>
                        <telerik:AjaxUpdatedControl ControlID="gridAuditData"></telerik:AjaxUpdatedControl>
                        <telerik:AjaxUpdatedControl ControlID="auditChart"></telerik:AjaxUpdatedControl>
                        <telerik:AjaxUpdatedControl ControlID="pagediv" LoadingPanelID="RadAjaxLoadingPanel1"></telerik:AjaxUpdatedControl>
                ...
                <%--<telerik:AjaxSetting AjaxControlID="timer">
                    <UpdatedControls>
                        <telerik:AjaxUpdatedControl ControlID="gridAuditData" LoadingPanelID="RadAjaxLoadingPanel1" />
                    </UpdatedControls>
                </telerik:AjaxSetting>--%>
                    ...
                    <telerik:RadSlidingPane ID="RadSlidingPane_Point" Width="200" runat="server" Title="站点" UndockText="收缩" DockText="固定" CollapseText="关闭"
                        ...
                        <div style="padding-top: 10px;">
                            <div style="width: 100%; height: 28px; color: #fff; background-color: #3A94D3;">
                                <div style="padding-top: 6px; padding-left: 6px;">仪器</div>
                            </div>
                        ...
                        <div id="GridDiv">
                            <telerik:RadGrid ID="gridAuditData" runat="server" GridLines="None"
                                ...
                                <MasterTableView GridLines="None" TableLayout="Fixed" CommandItemDisplay="None" IsFilterItemExpanded="False" ClientDataKeyNames="PointID,DataDateTime"
                                    InsertItemPageIndexAction="ShowItemOnCurrentPage" ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" NoMasterRecordsText="没有数据" HeaderStyle-HorizontalAlign="Center">
        ...
        <div style="display: none">
            ...
            <asp:HiddenField ID="InsIDHidden" runat="server" Value="" />
            ...
            <script type="text/javascript">
                ...
                function LoadingData() {
                    try {
                        ...
                        var pointType = "1";//是否超级站，1：是，0：否
                        AjaxLoadingMutilFactor(facCode, StartTime, EndTime, pointId, pointType, '<%=Session["applicationUID"]%>');
                    } catch (e) {
                    }
                }
                ...
                function ModifyAuditData(flag) {
                    if (flag != "-1") {
                        ...
                        AjaxAuditOperateData(PointID.join(";"), DataTime.join(";"), FactorCode.join(";"), NewData.join(";"), '<%=Session["applicationUID"]%>', reason, '<%=Session["UserGuid"]%>', "AuditAjaxHandler.ashx?DataType=ModifyAuditDataNew&flag=" + flag);
                    } else {
                        CellCancel();
                    }
                    windowState = null;
                ...
                //一行数据恢复到原始数据
                function RestorAuditDataRow() {
                    var reason = document.getElementById('auditReason').value;
                    $('#AuditSubmitDiv').css("display", "");
                    AjaxAuditOperateData(PointID.join(";"), DataTime.join(";"), document.getElementById('FactorCodes').value, NewData.join(";"), '<%=Session["applicationUID"]%>', reason, '<%=Session["UserGuid"]%>', "AuditAjaxHandler.ashx?DataType=RestoreAuditData");
                    //ClearSelectedInfo();
                }
                //审核提交
                function SubmitAuditClicked(sender, args) {
                    ...
                    var EndTime = $find("<%=RadDatePickerEnd.ClientID%>").get_selectedDate().format("yyyy/M/dd");
                    var pointId = $("#PointIDHidden").val();
                    var InsId = $("#InsIDHidden").val();
                    AjaxSubmitAudit(StartTime, EndTime, pointId, InsId, '<%=Session["applicationUID"]%>', '<%=Session["UserGuid"]%>', '<%=Session["DisplayName"]%>', '<%=ViewState["pointType"]%>');
                }
                //刷新列表
                ...
                function RadContextMenuClicked(sender, eventArgs) {
                    ...
                    switch (menuItemValue) {
                        ...
                        case "恢复":
                            ...
                            break;
                        case "时间段恢复":
                            window.radopen("AuditReason.aspx?operator=restoreRow", "WriteAuditReason"); //数据恢复
                            break;
                        case "时间段无效":
