    ...
    public partial class InteriorLabel : SmartEP.WebUI.Common.BasePage
    {
        ...
        public void BindGrid(string type)
        {
            ...
            try
            {
                ...
                if (uunit.SelectedValue == "ppb")
                {
                    sql = string.Format(@"SELECT  PollutantCode FROM [Standard].[TB_PollutantCode] where VOCType='2' and IsUseOrNot='1'");
                }
                else
                {
                    sql = string.Format(@"SELECT  PollutantCode FROM [Standard].[TB_PollutantCode] where VOCType='2' and IsUseOrNot='0'");
                }
