<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InstrumentFaultList.aspx.cs" Inherits="SmartEP.WebUI.Pages.EnvAir.Instrument.InstrumentFaultList" %>
...
<body>
    <form id="form1" runat="server">
        <telerik:RadScriptBlock ID="RadScriptBlock1" runat="server">
            <script type="text/javascript" language="javascript">
                ...
                function Refresh_Grid(args) {
                    if (args) {
                        var MasterTable = $find("<%= grdInstrumentFault.ClientID %>").get_masterTableView();
                        MasterTable.rebind();
                    }
                }
                ...
                function gridRTB_ClientButtonClicking(sender, args) {
                    ...
                    switch (CurrentBtnCommandName) {
                        case "InitInsert":
                            {
                                //增加
                                window.radopen("InstrumentFaultAdd.aspx?ObjectType=" + '<%=objectType%>', "InstrumentFault");
                ...
                function ShowEditForm(Id) {
                    window.radopen("InstrumentFaultAdd.aspx?RowGuid=" + Id + "&&ObjectType=" + '<%=objectType%>', "InstrumentFault");
                }

                //行详情按钮
                function ShowDetailForm(Id) {
                    window.radopen("InstrumentFaultAdd.aspx?RowGuid=" + Id, "InstrumentFault");
                }
                //行双击事件
                function OnRowDblClick(sender, args) {
                    ...
                    if (selectKeyValues != null && selectKeyValues["OfflineSettingUid"] != null) {
                        window.radopen("InstrumentFaultEdit.aspx?OffLineSettingUid=" + selectKeyValues["OfflineSettingUid"], "InstrumentFault");
                    }
                }
            </script>
        ...
        <telerik:RadAjaxManager ID="RadAjaxManager1" runat="server" UpdatePanelsRenderMode="Inline">
            <AjaxSettings>
                <telerik:AjaxSetting AjaxControlID="grdInstrumentFault">
                    <UpdatedControls>
                        <telerik:AjaxUpdatedControl ControlID="grdInstrumentFault" LoadingPanelID="RadAjaxLoadingPanel1" />
                    </UpdatedControls>
                </telerik:AjaxSetting>
        ...
        <telerik:RadSplitter ID="splitter" runat="server" Orientation="Horizontal" Height="100%" BorderWidth="0"
            ...
            <telerik:RadPane ID="paneGrid" runat="server" Width="100%" Height="100%" Scrolling="None"
                BorderWidth="0" BorderStyle="None" BorderSize="0">
                <telerik:RadGrid ID="grdInstrumentFault" runat="server" GridLines="None" Height="100%" Width="100%"
                    ...
                    OnNeedDataSource="grdInstrumentFault_NeedDataSource" OnItemDataBound="grdInstrumentFault_ItemDataBound" OnDeleteCommand="grdInstrumentFault_DeleteCommand"
        ...
        <telerik:RadWindowManager ID="RadWindowManager1" runat="server" VisibleStatusbar="false" Behaviors="Move,Pin,Reload,Close" EnableShadow="true">
            <Windows>
                <telerik:RadWindow ID="InstrumentFault" runat="server" Height="380px" Width="680px" Skin="Outlook" IconUrl="~/App_Themes/Office2007/images/RadGridHeaderBg2.png"
                    Title="仪器故障" ReloadOnShow="true" ShowContentDuringLoad="false" Modal="true" />
            </Windows>
