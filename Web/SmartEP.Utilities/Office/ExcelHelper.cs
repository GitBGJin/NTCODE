    ...
    public class ExcelHelper
    {
        ...
        public static void DataTable3Excel(System.Data.DataTable dtData, String FileName)
        {
            ...
            if (dtData != null)
            {
                ...
                //strWriter = new StringWriter(myCItrad);
                htmlWriter = new HtmlTextWriter(strWriter);
        ...
        public static void DataTableToExcel(DataTable table, string fileName, string sheetName, Page page)
        {
            ILog log = LogManager.GetLogger("FileLogging");
            //fileName = fileName + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
            fileName = fileName + ".xls";
            //完全路径
            String savePath = page.Server.MapPath("~/") + "\\Files\\TempFile\\Excel\\" + fileName;
            HttpResponse curResponse = HttpContext.Current.Response;
            if (CreateExcel(table, savePath, sheetName))
            {
                ...
                if (DownloadFile.Exists)
                {
                    ...
                    //curResponse.ContentType = "application/ms-excel";
                    curResponse.ContentType = "text/html";
                    log.Info("导出类型：" + curResponse.ContentType);
        ...
        /// <summary>
        /// DataTable导出到Excel(不带分秒)
        /// </summary>
        /// <param name="Table"></param>
        /// <param name="fileName"></param>
        /// <param name="sheetName"></param>
        /// <param name="page"></param>
        public static void DataTableToNoHExcel(DataTable table, string fileName, string sheetName, Page page)
        {
            fileName = fileName + DateTime.Now.ToString("yyyyMMdd") + ".xls";
            //完全路径
            String savePath = page.Server.MapPath("~/") + "\\Files\\TempFile\\Excel\\" + fileName;
            HttpResponse curResponse = HttpContext.Current.Response;
            if (CreateExcel(table, savePath, sheetName))
        ...
        /// <returns></returns>
        public static bool CreateExcel(DataTable Table, string fileName, string sheetName)
        {
            try
            {
                ...
                designer.Workbook.Worksheets[0].Cells.StandardWidth = 15;

                //设置单元格样式
                Aspose.Cells.Style cellStyle = designer.Workbook.Styles[designer.Workbook.Styles.Add()];
                //文字字体
                ...
                Worksheet sheet = (Worksheet)designer.Workbook.Worksheets[0];
                //添加标题
                for (var iCol = 0; iCol < Table.Columns.Count; iCol++)
                {
                    sheet.Cells[0, iCol].PutValue(Table.Columns[iCol].ColumnName);
                    sheet.Cells[0, iCol].SetStyle(cellStyle);
                }
                //sheet.Cells.SetRowHeight(0, 24);
                //生成数据行
                for (int iRow = 0; iRow < Table.Rows.Count; iRow++)
                {
                    for (int iCol = 0; iCol < Table.Columns.Count; iCol++)
                    {
                        Cell cell = sheet.Cells[1 + iRow, iCol];
                        cell.SetStyle(cellStyle);
                     
                        object value = Table.Rows[iRow][iCol];
                        if (value == DBNull.Value)
                            continue;
                        Type type = Table.Columns[iCol].DataType;
                        ...
                        {
                            cell.PutValue(Convert.ToDateTime(value).ToString("yyyy-MM-dd HH:mm:ss"));
                    ...
                    //sheet.Cells.SetRowHeight(1 + iRow, 24);
                }
                //保存文件
