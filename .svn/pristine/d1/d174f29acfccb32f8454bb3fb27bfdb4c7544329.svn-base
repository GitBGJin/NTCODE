...
namespace SmartEP.WebUI.Pages.EnvAir.Dock
{
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public partial class OnlineInfo : SmartEP.WebUI.Common.BasePage
    {
        /// <summary>
        /// 数据处理服务
        /// </summary>
        ...
        private void InitControl()
        {
            ...
            {
                if (comboCityProper.Items[i].Text == "全市")
                {
                    comboCityProper.Items.Remove(comboCityProper.Items[i]);
                }
        ...
        private void BindData()
        {
            ...
            hdMolde.Value = radlDataType.SelectedValue;
            #endregion

            #region 绑定数据

            DataTable dvOnlineRate = null;
            ...
            //airRealTimeOnlineState.GetRealTimeOnlineStateDataPager(netWorkType, strPoints, strFactors, pageSize, pageNo, out recordTotal, out dvOnlineRate);
            DataOnlineService dataOnline_service = new DataOnlineService();
            dvOnlineRate = dataOnline_service.GetTotalCount(ApplicationType.Air, strPoints, SmartEP.Core.Enums.EnumMapping.ParseEnum<PollutantDataType>(radlDataType.SelectedValue));
            hdAirOnlineInfo.Value = ToJson(dvOnlineRate);
