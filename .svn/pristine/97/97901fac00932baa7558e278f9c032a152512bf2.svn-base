...
namespace SmartEP.Data.SqlServer.MonitoringBusiness.Water
{
    ...
    public class WeekReportDAL
    {
        ...
        /// 数据库表名
        /// </summary>
        private string tableName = "WaterReport.TB_WeekReport";
        #endregion
        ...
        {
            tableName = SmartEP.Data.Enums.EnumMapping.GetAutoMonitorTableName(ApplicationType.Water, PollutantDataType.Week);
            connection = SmartEP.Data.Enums.EnumMapping.GetConnectionName(ApplicationType.Water, PollutantDataType.Week);
        }
        #endregion
            ...
            , int yearFrom, int weekOfYearFrom, int yearTo, int weekOfYearTo, int pageSize, int pageNo, out int recordTotal, string orderBy = "PointId,Year,WeekOfYear")
        {
            ...
            string fieldName = "PointId,Year,WeekOfYear" + factorSql + factorDataFlagSql + factorAuditFlagSql + factorEQISql + factorGradeSql + wqSql;
        ...
        public DataView GetExportData(string[] portIds, string[] factors
            , int yearFrom, int weekOfYearFrom, int yearTo, int weekOfYearTo, string orderBy = "PointId,Year,WeekOfYear")
        {
            tableName = "dbo.V_Water_WeekReport";
            orderBy = !string.IsNullOrEmpty(orderBy) ? orderBy : "PointId,Year,WeekOfYear";
            int recordTotal = 0;
            int pageSize = 99999999;
            int pageNo = 0;
            ...
            //}
            orderBy = string.IsNullOrEmpty(orderBy) ? "PointId,Year,WeekOfYear" : orderBy;
            string fieldName = "rowNum as '序号',PointId,portName,Year as '年',WeekOfYear as '周'" + factorSql + factorFlagSql;
            string groupBy = "PointId,portName,Year,WeekOfYear";
            //string where = string.Format(" Year>='{0}' AND Year<='{1}' AND WeekOfYear>='{2}' AND WeekOfYear<='{3}' ", yearFrom, yearTo, weekOfYearFrom, weekOfYearTo) + portIdsStr;
            //把周数据抽象成数字
            int weekFrom = yearFrom * 1000 + weekOfYearFrom;
            int weekTo = yearTo * 1000 + weekOfYearTo;
