...
namespace SmartEP.Service.BaseData.Channel
{
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public class AirPollutantService : IPollutant
    {
        //通道仓储层
        private PollutantCodeRepository g_Repository = new PollutantCodeRepository();
        //大气污染物类型值
        private string pollutantTypeValue = SmartEP.Core.Enums.EnumMapping.GetPollutantTypeValue(PollutantTypeValue.Air);
        private string applicationUid = SmartEP.Core.Enums.EnumMapping.GetApplicationValue(ApplicationValue.Air);
        #region 增删改
        /// <summary>
        /// 增加通道
        /// </summary>
        ...
        /// <param name="channelName"></param>
        /// <returns></returns>
        public IQueryable<PollutantCodeEntity> RetrieveListByCalAQI()
        {
            return g_Repository.Retrieve(p => p.PollutantTypeUid == pollutantTypeValue && p.IsCalEQIOrNot == true).OrderByDescending(p=>p.OrderByNum);
        }
