namespace SmartEP.Service.ReportLibrary.Air
{
    ...
    /// <summary>
    /// Summary description for AirQualityStatisticalMonthReport.
    /// </summary>
    public partial class AirQualityStatisticalMonthReportService : Telerik.Reporting.Report
    {
    DataQueryByDayService dataByDayService = new DataQueryByDayService();
        ...
        public AirQualityStatisticalMonthReportService()
        {
            InitializeComponent();

        }

        public void BindingPortReport(string[] pointID, DataTable dtStatistical,DataTable dtData, string[] factorField, string[] factorName, IList<IPollutant> factors, DateTime beginTime, DateTime endTime)
        {
            ...
            double sourceY = 0;
            GetHtmlTextBox(0, sourceY, tableWidth, 0.5, 12, true, "center", "空气质量统计月报");
            for (int i = 0; i < pointID.Length; i++)
            {
                ...
                sourceY += 0.5;
                GetHtmlTextBox(0, sourceY, tableWidth, 0.5, 10, false, "left", string.Format(@"      测点名称  {0}                                                                                    月份{1}               							" ,pointName, beginTime.Month));
                sourceY += 0.6;
                CreateTableAutoMonthReport(this.detail, dv.ToTable(), resultStatistical.ToTable(), CommonField, CommonName, factors, sourceY);
