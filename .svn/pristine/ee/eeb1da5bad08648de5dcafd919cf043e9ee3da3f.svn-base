...
namespace NTDataProcessApplication
{
    ...
    public class AQICalculateService
    {
        ...
        /// IAQIH:对应于BPH的指数限值
        /// <returns></returns>
        public int? GetIAQI(string PollutantCode, decimal? PollutantValue, int TimeType)
        {
            try
            {
            ...
            try
            {
                ...
                switch (TimeType)
                {
                    case 1:
                        {
                            #region 1小时均值
                            DataTable dt = d_AQIDAL.GetHourRegionValue(PointIds, PollutantCode, Tstamp);
                    ...
                    case 8:
                        {
                            if (PollutantCode == "a05027")
                            {
                                ...
                                DataTable dt = d_AQIDAL.GetO3_8NTRegionValue(PointIds, Tstamp);
                                int EffectCount = dt.Rows.Count;
                                if (count < 4)
                                {
                                    if (EffectCount * 1.0 / count >= 0.5)
                            ...
                            {
                                #region 臭氧日最大8小时均值
                                DataTable dt = d_AQIDAL.GetO3_8RegionValue(PointIds, Tstamp);
                                int EffectCount = dt.Rows.Count;
                                if (count < 4)
                                {
                                    if (EffectCount * 1.0 / count >= 0.5)
                    ...
                    case 24:
                        {
                            #region 24小时均值
                            DataTable dt = d_AQIDAL.GetDayRegionValue(PointIds, PollutantCode, Tstamp);
                            int EffectCount = dt.Rows.Count;
                            if (count < 4)
                            {
                                if (EffectCount * 1.0 / count > 0.5)
            ...
            try
            {
                ...
                switch (TimeType)
                {
                    case 1:
                        {
                            #region 小时均值
                            DataTable dt = d_AQIDAL.GetHourRegionValueByTime(PointIds, PollutantCode, StartTime, EndTime);
                            int EffectCount = dt.Rows.Count;
                            if (count < 4)
                            {
                                if (EffectCount * 1.0 / count > 0.5)
                    ...
                    case 8:
                        {
                            #region 臭氧日最大8小时均值
                            DataTable dt = d_AQIDAL.GetO3_8RegionValueByTime(PointIds, StartTime, EndTime);
                            int EffectCount = dt.Rows.Count;
                            if (count < 4)
                            {
                                if (EffectCount * 1.0 / count >= 0.5)
                    ...
                    case 24:
                        {
                            #region 日均值
                            DataTable dt = d_AQIDAL.GetDayRegionValueByTime(PointIds, PollutantCode, StartTime, EndTime);
                            int EffectCount = dt.Rows.Count;
                            if (count < 4)
                            {
                                if (EffectCount * 1.0 / count > 0.5)
