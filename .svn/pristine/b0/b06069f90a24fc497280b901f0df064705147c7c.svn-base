...
namespace SmartEP.Service.DataAnalyze.Air.DataQuery
{
    ...
    public class DataQueryByYearService
    {
        ...
        /// <param name="orderBy">排序方式（字段：PointId,Year）</param>
        /// <returns></returns>
        public DataView GetYearDataPagerRegion(string[] portIds, IList<IPollutant> factors, int yearFrom
            , int yearTo, int pageSize, int pageNo, out int recordTotal, string orderBy="")
        {
          recordTotal = 0;
          if (factors.IsNotNullOrDBNull())
            return YearData.GetYearDataPagerRegion(portIds, factors.Select(p => p.PollutantCode).ToArray(), yearFrom, yearTo, pageSize, pageNo, out recordTotal, orderBy);
          return null;
        }
        /// <summary>
        /// 取得虚拟分页查询数据和总行数(行转列数据)
        ...
        /// <param name="orderBy">排序方式（字段：PointId,Year）</param>
        /// <returns></returns>
        public DataView GetYearDataPagersRegion(string[] portIds, string[] factors, int yearFrom
            , int yearTo, int pageSize, int pageNo, out int recordTotal, string orderBy = "")
        {
          recordTotal = 0;
          if (factors.IsNotNullOrDBNull())
            return YearData.GetYearDataPagerRegion(portIds, factors, yearFrom, yearTo, pageSize, pageNo, out recordTotal, orderBy);
          return null;
        }
        /// <summary>
        /// 取得虚拟分页查询数据和总行数(行转列数据)
