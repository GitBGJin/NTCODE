namespace SmartEP.Service.ReportLibrary.Air
{
    ...
    /// <summary>
    /// Summary description for PointReport.
    /// </summary>
    public partial class PointReport : Telerik.Reporting.Report
    {
        ...
        public void BindingPortReport(string[] pointID, DataTable dtStatistical, DataTable dtData, string[] factorField, string[] factorName, IList<IPollutant> factors, DateTime beginTime, DateTime endTime, string reportType, string week = "")
        {
            DataView dv = dtData.DefaultView;
            string[] CommonField = ("DateTime" + (factorField.Length > 0 ? ";" + string.Join(";", factorField) : "")).Split(';');
            string[] CommonName = ("日期" + (factorName.Length > 0 ? ";" + string.Join(";", factorName) : "")).Split(';');
            double sourceY = 0;
            for (int i = 0; i < pointID.Length; i++)
            {
                ...
                GetHtmlTextBox(0, sourceY, tableWidth, 0.5, 12, true, "center", pointName + "大气监测" + reportType + "报表");
                sourceY += 0.5;
                GetHtmlTextBox(0, sourceY, tableWidth, 0.5, 10, false, "left", string.Format(@"测点名称：{0}", pointName));
                GetHtmlTextBox(0, sourceY, tableWidth, 0.5, 10, false, "right", string.Format(@"日期：{1}", pointName, GetDateFormat(endTime, reportType, week)));
                sourceY += 0.6;
                CreateTableAutoMonthReport(this.detail, dv.ToTable(), resultStatistical.ToTable(), CommonField, CommonName, factors, sourceY, reportType);
                sourceY += tdcolumnHight * dv.Count + tdcolumnHight * resultStatistical.Count + 1;
            }
        }

        private string GetDateFormat(DateTime time, string reportType, string week)
        {
            string date = "";

            if (reportType.Equals("日"))
            {
                //string weekname = time.Date.DayOfWeek.ToString();
                string[] weekdays = { "星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六" };
                string weekname = weekdays[Convert.ToInt32(DateTime.Now.DayOfWeek)];
                date = time.ToString("yyyy年MM月dd日"+" "+weekname);
            }
            else if (reportType.Equals("月"))
                date = time.ToString("yyyy年MM月");
            else if (reportType.Equals("周"))
                date = time.ToString("yyyy年MM月") + "  " + week;
            else if (reportType.Equals("年"))
                date = time.ToString("yyyy年") + "";
            return date;
        }
        /// <summary>
        /// 创建表格
        ...
        /// <returns></returns>
        public Telerik.Reporting.Table CreateTableAutoMonthReport(DetailSection detail, DataTable dt, DataTable dtRow, string[] columnField, string[] columnName, IList<IPollutant> factors, double souceY, string reportType = "")
        {
            ...
            for (int i = 0; i <= ColCount - 1; i++)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    if (i < 1)
                    {
                        if (reportType.Equals("年"))
                            textBoxTable.Value = "=SmartEP.Service.ReportLibrary.Air.PointReport.TimeFormat(Fields.Year" + ",Fields.MonthOfYear)";
                        else
                ...
                if (dtRow.Rows.Count > 0)
                {
                    for (int k = 0; k < dtRow.Rows.Count; k++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            tb.Body.SetCellContent(dt.Rows.Count>0?k + 1:k, i, textBoxTable);
                        }
                        else
                        {
                            try
