...
{
    /// <summary>
    /// 名称：AuditWaterInfectantByHourDAL.cs
    /// 创建人：李飞
    /// 创建日期：2015-08-30
    /// 维护人员：
    /// 最新维护人员：
    ...
    /// 版权所有(C)：江苏远大信息股份有限公司
    /// </summary>
    public class AuditWaterInfectantByHourDAL
    {
        #region << 变量 >>
        /// <summary>
        /// 数据库处理类
        ...
        /// 数据库表名
        /// </summary>
        private string tableName = "Audit.TB_AuditWaterInfectantByHour";
        #endregion
        ...
        /// 构造函数
        /// </summary>
        public AuditWaterInfectantByHourDAL()
        {
        ...
        public DataView GetDataView(string[] portIds, string[] factors, DateTime dtStart, DateTime dtEnd, bool isAllDate = false)
        {
            try
            {
                ...
                string sql = string.Format(@"
                   SELECT * FROM (SELECT dayInfo.PointId
                        ...
                        {2}
                        {3}
                    FROM {4} AS hourInfo WITH(NOLOCK)
                    INNER JOIN Audit.TB_AuditStatusForDay AS dayInfo WITH(NOLOCK)
	                    ON hourInfo.AuditStatusUid = dayInfo.AuditStatusUid
                    INNER JOIN Audit.TB_AuditWaterStatusForHour AS hourStatus WITH(NOLOCK)
                    ...
                    GROUP BY dayInfo.PointId,hourInfo.DataDateTime
                     ) AS A ORDER BY A.PointId,A.DataDateTime DESC
                ...
                if (isAllDate && portIds != null && portIds.Length > 0)
                {
                    sql = string.Format(@"
                                    ...
                                    SELECT allInfo.PointID
                                        ...
                                        {3}
                                        {4}
                                    FROM dbo.F_GetAllDataByHour('{5}',',','{6}','{7}') AS allInfo WITH(NOLOCK)
                                    LEFT JOIN hourData WITH(NOLOCK)
        ...
        {
            string begintime = Convert.ToDateTime(beginTime.ToString()).ToString("yyyy-MM-dd HH:mm:ss");
            string endtime = Convert.ToDateTime(endTime.ToString()).ToString("yyyy-MM-dd HH:mm:ss");
            ...
            if (endTime > beginTime)
            {
                ...
                for (int i = 1; i <= days; i++)
                {
                    ...
                    string newtimes = Convert.ToDateTime(newTimes.ToString()).ToString("yyyy-MM-dd HH:mm:ss");
        ...
        public DataTable AuditFlagStatisticsByPointId(string application, string point, DateTime beginTime, DateTime endTime)
        {
            ...
            strSql.Append(" where [Date] = '" + begintime + "' and ApplicationUid='" + application + "' ");
            strSql.Append(" group by [Date],PointId ) as dayInfo0 on smp0.PointId = dayInfo0.PointId ");
            strSql.Append(" where smp0.ApplicationUid='" + application + "' ");
            strSql.Append(" and smp0.PointId in ("+point+") ");
            strSql.Append(" and smp0.EnableOrNot=1 and smp0.ShowOrNot=1 ");
            if (endTime > beginTime)
            {
                TimeSpan ts = endTime - beginTime;
                ...
                for (int i = 1; i <= days; i++)
                {
                    ...
                    strSql.Append(" where [Date] = '" + newtimes + "' and ApplicationUid='" + application + "' ");
                    strSql.Append(" group by [Date],PointId ) as dayInfo" + i.ToString() + " on smp" + i.ToString() + ".PointId = dayInfo" + i.ToString() + ".PointId ");
                    strSql.Append(" where smp" + i.ToString() + ".ApplicationUid='" + application + "' ");
                    strSql.Append(" and smp" + i.ToString() + ".PointId in ("+point+") ");
                    strSql.Append(" and smp" + i.ToString() + ".EnableOrNot=1 and smp" + i.ToString() + ".ShowOrNot=1 ");
                }
            }
            strSql.Append(" ) as t order by t.OrderByNum desc,t.PointId asc,t.Date desc ");
        ...
        public void AuditFactorHourStatusUpdate(int PointID, DateTime beginTime, DateTime endTime, string[] factors, string UserName)
        {
            string sql = string.Format(@"UPDATE hourStatus 
	                                    ...
	                                    WHERE adtData.PointId IS NOT NULL"
                                      , factors.Length, UserName, PointID, beginTime, endTime);
            g_DatabaseHelper.ExecuteNonQuery(sql, connection);
        }
