...
{
    public partial class AuditSetting_Super : SmartEP.WebUI.Common.BasePage
    {
        /// <summary>
        /// 系统配置数据接口
        /// </summary>
        ...
        public void LoadSiteTypeData()
        {
            ...
            ContrlUidLists = ContrlUidLists.OrderBy(p => p.SortNumber);
        ...
        private void BindTable()
        {
            ...
            dt.Columns.Add("InsUid", typeof(string));
            ...
            //IQueryable<MonitoringPointEntity> PointLists = g_MonitoringPointService.Retrieve(x => x.ApplicationUid == ApplicationUid && x.ContrlUid == ContrlUid && (Boolean)x.EnableOrNot && (Boolean)x.ShowOrNot && (Boolean)x.MonitoringPointExtensionForEQMSAirEntity.SuperOrNot).OrderByDescending(x => x.OrderByNum);
            //List<string> PointUids = new List<string>();
            IQueryable<MonitoringPointEntity> PointLists = g_MonitoringPointService.Retrieve(x => x.ApplicationUid == ApplicationUid && x.MonitoringPointUid == "f2369c1f-95e1-4f5d-9045-9c185ce8727a" && (Boolean)x.EnableOrNot && (Boolean)x.ShowOrNot && (Boolean)x.MonitoringPointExtensionForEQMSAirEntity.SuperOrNot).OrderByDescending(x => x.OrderByNum);
            IQueryable<InstrumentEntity> instrumentList = instrumentService.RetrieveListByPointUid("f2369c1f-95e1-4f5d-9045-9c185ce8727a");
            instrumentList = instrumentList.Where(p => p.RowGuid != "6cd5c158-8a79-4540-a8b1-2a062759c9a0");//剔除超级站常规参数
            List<string> InsUids = new List<string>();
            MonitoringPointEntity mp = PointLists.FirstOrDefault();
            //if (PointLists != null)
            if (instrumentList != null)
            {
                //foreach (MonitoringPointEntity point in PointLists)
                foreach (InstrumentEntity ins in instrumentList)
                {
                    ...
                    //dt.Rows.Add(dr);
                    if (!InsUids.Contains(ins.InstrumentTypeUid))
                    {
                        InsUids.Add(ins.InstrumentTypeUid);
                    }
                    DataRow dr = dt.NewRow();
                    dr["InsUid"] = ins.RowGuid;
                    dr["InsName"] = ins.InstrumentName;
                    dr["PointUid"] = mp.MonitoringPointUid;
                    dr["PointName"] = mp.MonitoringPointName;
                    dr["ContrlUid"] = mp.ContrlUid;
                    dt.Rows.Add(dr);
                }
            }
            //测点对应状态因子
            ...
            IQueryable<V_Point_InstrumentChannelEntity> PollutantLists = g_PollutantService.Retrieve(x => x.TypeUid == PollutantTupeUid && x.ApplicationUid == ApplicationUid).OrderByDescending(x => x.OrderByNum);
            //审核配置因子
            ...
            IQueryable<AuditMonitoringPointPollutantEntity> AuditLists = g_AuditMonitoringPointService.DetailRetrieve(x => "f2369c1f-95e1-4f5d-9045-9c185ce8727a".Contains(x.AuditMonitoringPointEntity.MonitoringPointUid) && x.AuditMonitoringPointEntity.ApplicationUid == ApplicationUid && x.AuditMonitoringPointEntity.PointType == PointType);
            AuditMonitoringPointPollutantEntity[] AuditList = AuditLists.ToArray();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string InsUid = Convert.ToString(dr["InsUid"]);
                ...
                if (PollutantLists != null)
                {
                    ...
                    {
                        if ("f2369c1f-95e1-4f5d-9045-9c185ce8727a" == pollutant.MonitoringPointUid && pollutant.InstrumentUid == InsUid)
                        {
                    ...
                    if (AuditLists != null)
                    {
                        ...
                        {
                            if ("f2369c1f-95e1-4f5d-9045-9c185ce8727a" == audit.AuditMonitoringPointEntity.MonitoringPointUid)
                            {
        ...
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                string PointUid = ((HiddenField)rptList.Items[i].FindControl("HiddenField1")).Value;
                PointUids.Add(PointUid);
                string ContrlUid = ((HiddenField)rptList.Items[i].FindControl("HiddenField3")).Value;
